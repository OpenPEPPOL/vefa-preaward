...
This BIS is based on work done by {CENBII} and the European Commision for ESPD, and business rules and code lists used are inherited from the follwing:
...
IMPORTANT: The profile document and syntax implementation guides from {CENBII}, as well as the {ESPD_model} and {ESPD_docs} must be considered when implementing BIS Tendering.
